//     FileSourceModule(std::string name) : fileSelect("", { "Wav IQ файли (*.wav)", "*.wav", "Усі файли", "*" }) {
...
SDRPP_MOD_INFO{
    /* Name:            */ "file_source",
    /* Description:     */ "Wav file source module for Aster",
    /* Author:          */ "DMH",
    /* Version:         */ 0, 1, 1,
    /* Max instances    */ 1
};
...
public:
    FileSourceModule(std::string name) : fileSelect("", { "Wav IQ Files (*.wav)", "*.wav", "All Files", "*" }) {
        ...
        sigpath::sourceManager.registerSource("Файл", &handler);
    }

    ~FileSourceModule() {
        stop(this);
        sigpath::sourceManager.unregisterSource("Файл");
    }
...
private:
    ...
    double getFrequency(std::string filename) {
        ...
        flog::info("FileSourceModule freqStr '{0}'", freqStr);        
