...
bool Menu::draw(bool updateStates) {
    ...
    for (MenuOption_t& opt : order) {
        ...
        if (displayedCount == insertBefore && !draggedMenuName.empty()) {
            ...
            if (items[draggedOpt.name].inst != NULL) {
                ...
                ImGui::Checkbox(("1##_menu_checkbox_" + draggedOpt.name).c_str(), &enabled);
        ...
        if (menuDragged && !locked) {
            ...
            ImGui::SetTooltip("Переміщення заблоковано!");
        }
        ...
        if (ImGui::CollapsingHeader((opt.name + "##sdrpp_main_menu").c_str())) {
            if (item.inst != NULL) {
                ...
                if (ImGui::Checkbox(("2##_menu_checkbox_" + opt.name).c_str(), &enabled)) {
                    if (opt.name != "Канал приймання") {
                        flog::info("ImGui::CollapsingHeader 2 menu_checkbox_ - {0} !", opt.name);
            ...
            // flog::info("101 {0} 4", rawId);
            item.drawHandler(item.ctx); // was ERROR
            ImGui::Spacing();
        }
            ...
            ImGui::SetCursorPosY(pos.y + checkboxOffset.y);
            bool enabled = item.inst->isEnabled();
            if (ImGui::Checkbox(("3##_menu_checkbox_" + opt.name).c_str(), &enabled)) {
                if (opt.name != "Канал приймання") {
                    flog::info("ImGui::CollapsingHeader 3 menu_checkbox_ - {0} ({1}) !", opt.name, item.inst->isEnabled());
                    enabled ? item.inst->enable() : item.inst->disable();
                    changed = true;
                }
            }
    ...
    if (insertBefore == displayedCount && !draggedMenuName.empty()) {
        ...
        if (items[draggedOpt.name].inst != NULL) {
            ...
            ImGui::SetCursorPosY(pos.y + checkboxOffset.y);
            bool enabled = items[draggedOpt.name].inst->isEnabled();
            ImGui::Checkbox(("4##_menu_checkbox_" + draggedOpt.name).c_str(), &enabled);
            ImGui::SetCursorPos(pos);
        }
        style::endDisabled();
        window->DrawList->AddRect(posMin, posMax, textColor, 0.0f, 0, style::uiScale);
