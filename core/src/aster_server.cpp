...
namespace aster_server {
    ...
    AServer::AServer() {
        _run=false;
        // Start worker thread
        workerThread = std::thread(&AServer::worker, this);
    }
    ...
    void AServer::worker() {
        flog::info("AServer::worker = {0}", bytes);

        int bytes =0;
        while (true) {
            if(!_run) continue;
            bytes++;
            flog::info("worker= {0}", bytes);
            sleep(1);
            if(bytes==10) {
                flog::info("worker 10= {0}", bytes);
                bytes = 0;
            }
        }
    }
    ...
    void _packetHandler(int count, uint8_t* buf, void* ctx) {
        ...
        if (hdr->type == PACKET_TYPE_COMMAND && hdr->size >= sizeof(PacketHeader) + sizeof(CommandHeader)) {
            ...
            commandHandler((aster_server::Command)chdr->cmd, &buf[sizeof(PacketHeader) + sizeof(CommandHeader)], hdr->size - sizeof(PacketHeader) - sizeof(CommandHeader));
        }
        else {
    ...
    void drawMenu() {
        ...
        if (SmGui::Combo("##sdrpp_server_src_sel", &sourceId, sourceList.txt)) {
            flog::info("TRACE Aster_server! sourceId {0}:{1} Combo", sourceId, sourceList[sourceId]); 
            sigpath::sourceManager.selectSource(sourceList[sourceId]);
            core::configManager.acquire();
            ...
            flog::info("    AirspySourceModule enable freq {0}, def_freq {1}", def_freq, def_freq);
        }
