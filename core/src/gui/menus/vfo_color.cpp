...
namespace vfo_color_menu {
    ...
    void draw(void* ctx) {
        ImGui::BeginTable("Таблица колорів каналу приймання", 2);
        ImGui::TableNextRow();

        ImGui::TableSetColumnIndex(0);
        if (ImGui::Button("Автоколір#vfo_color", ImVec2(ImGui::GetContentRegionAvail().x, 0))) {
        ...
        if (ImGui::Button("Очистити все##vfo_color", ImVec2(ImGui::GetContentRegionAvail().x, 0))) {
            for (auto& [name, vfo] : gui::waterfall.vfos) {
                vfoColors[name] = ImVec4(1.0f, 1.0f, 1.0f, 1.0f);
                vfo->color = IM_COL32(255, 255, 255, 50);
                core::configManager.acquire();
        ...
        ImGui::BeginTable("Колір каналу приймання", 1, ImGuiTableFlags_RowBg | ImGuiTableFlags_Borders);
        for (auto& [name, vfo] : gui::waterfall.vfos) {
            ImGui::TableNextRow();
            ImGui::TableSetColumnIndex(0);
