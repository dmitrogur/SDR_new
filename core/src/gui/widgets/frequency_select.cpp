...
void FrequencySelect::draw(bool disabled) {
    int _work = disabled;

    if (old_freq != (uint64_t)frequency && old_freq > 1000) {
        currSource = sourcemenu::getCurrSource(); //  sigpath::sourceManager::getCurrSource();
        // if (currSource == SOURCE_ARM) {
        // && !gui::mainWindow.getUpdateMenuSnd0Main(gui::mainWindow.getCurrServer())
        // flog::info("draw ___frequency {0}, old_freq {1}", frequency, old_freq);
        if(gui::mainWindow.getUpdateFreq()) {
            gui::mainWindow.setUpdateFreq(false);
        } else {
    ...
    if (!gui::mainWindow.lockWaterfallControls && _work == false) { //
...
void FrequencySelect::setFrequency(int64_t freq) {
    // old_freq= frequency;
    // flog::info("1. frequency ({0}), old_freq {1}", freq, old_freq);
    ...
    frequency = freq;
    // flog::info("2. frequency ({0}), old_freq {1}", freq, old_freq);
}