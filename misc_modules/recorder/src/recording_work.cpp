...
public:
    ...
    RecorderModule(std::string name) : folderSelect("%ROOT%/recordings"), folderSelect_akf("%ROOT%/recordings")
    {
        ...
        // rawIQStream = new dsp::stream<dsp::complex_t>();
        // Создаём фильтр + дециматор
        // iqFilter = new dsp::demod::pureIQ<dsp::complex_t>();
        // double bandwidth = sigpath::vfoManager.getBandwidth(gui::waterfall.selectedVFO);
        // iqFilter->init("pureIQ_rec", rawIQStream, 12500, 2500000, 96000);
        // pureIQSink.init(&stereoStream, complexHandler, this);
    ...
    std::string genWavFileName(const double current, const int _mode)
    {
        ...
        sprintf(yearStr, "%02d", ltm->tm_year + 1900);
        // flog::info("radio. templ 1 = {0}", templ);
        ...
        // flog::info("radio. templ 2 = {0}, thisInstance {1}", templ, thisInstance);
    ...
    void postInit()
    {
        ...
        for (const auto &name : names)
        {
            ...
            // flog::warn("TRACE! RECORD! audioStreams name {0}", name);
        }
    ...
    void start()
    {
        ...
        // Используем блок для ограничения области видимости и времени жизни lock_guard
        {
            ...
            /*
            bool flag = false;
            // Стандартная логика с временной директорией
            std::string tmp_wavPath = wavPath + "/../tmp_recording";
            flag = makeDir(wavPath.c_str());
            flag &= makeDir(tmp_wavPath.c_str()); // Убеждаемся, что обе директории созданы
            mainWavPath = tmp_wavPath;
            expandedPath = expandString(tmp_wavPath + "/" + currWavFile);
            curr_expandedPath = expandString(wavPath + "/" + currWavFile);
            if (!flag)
            {
                flog::warn("There was an issue creating recording directories.");
            }
            */
            // =================================================================
...
private:
    ...
    void restart()
    {
        ...
        stop(true); // true, так как при рестарте мы обычно хотим сохранить старый файл
        /*
        if (recording)
        {
            stop();
    ...
    static void menuHandler(void *ctx)
    {
        ...
        */
        ...
        if (_this->Admin)
        {
            ...
            /*
            ImGui::NextColumn();
            if (ImGui::RadioButton(CONCAT("IQ##_recorder_mode2_", _this->name), _this->recMode == RECORDER_MODE_PUREIQ)) {
                _this->recMode = RECORDER_MODE_PUREIQ;
                config.acquire();
                config.conf[_this->name]["mode"] = _this->recMode;
                config.release(true);
                _this->sampleTypeId = _this->sampleTypes.valueId(wav::SAMP_TYPE_INT16); // ::SAMP_TYPE_INT16);
                config.acquire();
                config.conf[_this->name]["sampleType"] = _this->sampleTypes.key(_this->sampleTypeId);
                config.release(true);
            }
        ...
        */
        ...
        if (_this->recMode = RECORDER_MODE_AUDIO) {
            ...
            ImGui::FillWidth();
            if (ImGui::Combo(CONCAT("##_recorder_Sampling", _this->name), &_this->samplingRateId, _this->samplingRates.txt)) {
                config.acquire();
                config.conf[_this->name]["sampleRate"] = _this->samplingRates.key(_this->samplingRateId);
                config.release(true);
                std::string strSR = _this->samplingRates.name(_this->samplingRateId);
                flog::info("_this->sampleTypeId {0}, {1}, {2}",_this->samplingRateId,  _this->samplingRates.key(_this->samplingRateId), _this->samplingRates.name(_this->samplingRateId));
                _this->samplerate = (uint64_t)std::stoi(strSR);
                sigpath::sinkManager.setStreamSampleRate(_this->selectedStreamName, _this->samplerate);
            }
        }
        */
        ...
        if (_this->isARM)
        {
            ...
            else
            {
                ...
                if(_this->radioMode>0){
                    if(seconds>=_this->maxTimeRecordMin*60)
                        _this->_restart = true;
                }
                */
...
#define MAXLINE 1024
    ...
    void startServer_old()
    {
        // Инициализируем статус как "ожидание"
        analysisResultSignal.store(ANALYSIS_PENDING);
        flog::info("startServer: Waiting for analysis result for file {0}", currWavFile);
        ...
        int WAIT_SEC = maxRecShortDur_sec + 4;
        int vaport = SIport + NUM_INST;

        flog::info("startServer: Listening on port {0} for instance '{1}' (NUM_INST={2})", vaport, name, NUM_INST);
        // int addport = (NUM_INST > 0) ? (NUM_INST = NUM_INST - 1) : 0;
        // int vaport = SIport + NUM_INST;

        // Блокируемся и ждем UDP-пакет
        std::string info = "";
        info = net::listenUDP(hostname, vaport, WAIT_SEC, shutdownPipeFd[0]);

        if (info == "error") // Или более специфичная ошибка, если вы ее добавите
        {
            // Явно логируем ошибку привязки
            flog::error("Failed to bind UDP socket on port {0}. It might be already in use.", vaport);
            // analysisResultSignal.store(ANALYSIS_NOISE_OR_TIMEOUT);
            analysisResultSignal.store(ANALYSIS_ERROR_OR_TIMEOUT); // "HB"
        }
        else if (info == "") // Предположим, пустая строка - это таймаут
        {
            flog::info("Analysis timed out. No UDP packet received on port {0}.", vaport);
            // analysisResultSignal.store(ANALYSIS_NOISE_OR_TIMEOUT);
            analysisResultSignal.store(ANALYSIS_ERROR_OR_TIMEOUT); // "HB"
        }
        else
        {
            ...
            if (!import.is_null())
            {
                try
                {
                    // Используем локальную переменную, а не член класса
                    int localSignal = import["Signal"].get<int>();

                    if (localSignal > 0)
                    {
                        // Обрабатываем значение сигнала, если нужно
                        if (localSignal == 2 && import.contains("Comment"))
                        {
                            if (import["Comment"] == "HB")
                            {
                        ...
                        // Сообщаем результат: записываем конкретное значение сигнала
                        flog::info("Analysis result: SIGNAL DETECTED with value {0} for {1}", localSignal, currWavFile);
                        analysisResultSignal.store(localSignal);
                    }
                    else
                    {
                        // Это шум. Сообщаем об этом, записывая 0.
                        flog::info("Analysis result: NOISE DETECTED for {0}", currWavFile);
                        analysisResultSignal.store(ANALYSIS_NOISE_OR_TIMEOUT);
                    }
                }
        ...
        // Поток выполнил свою задачу и просто завершается.
    }
    ...
    void startServer2()
    {
        flog::info("startServer currWavFile= {0}", currWavFile);
        strcpy(hostname, host.c_str());
        // if (modeId == SINK_MODE_UDP) {

        std::string info = "";
        int WAIT_SEC = maxRecShortDur_sec + 2;
        Signal = -1;
        auto start_time = std::chrono::high_resolution_clock::now();
        int vaport = SIport + NUM_INST;
        info = net::listenUDP(hostname, vaport, WAIT_SEC);

        flog::info("startServer 2 currWavFile= {0}, vaport {1}", currWavFile, vaport);

        if (info != "" && info != "error")
        {
            // flog::info("info = {0}", info);
            auto now = std::chrono::high_resolution_clock::now();
            int time = (std::chrono::duration_cast<std::chrono::milliseconds>(now - start_time)).count(); //  - maxRecShortDur_sec*1000;
            json import = parseUdpData(info);
            if (!import.is_null())
            {
                try
                {
                    Signal = import["Signal"].get<int>();

                    if (Signal == 2)
                    {
                        // Проверка, есть ли ключ "Comment" и не равен ли он "DMR"
                        if (import.contains("Comment"))
                        {
                            // {"Channel": "va_20250629-1751199732416-171612500-12500-3-2-ЧМ.wav", "Signal": 2, "Probability": 1.0, "Comment": "HB" }
                            if (import["Comment"] == "HB")
                            {
                                Signal = 3; // NV
                            }
                    ...
                    flog::info("Time(ms) {0}. SignalInfo = {1}!", time, Signal);
                }
                catch (const std::exception &e)
                {
                    flog::warn("Time(ms) {0}. Ошибка разбора JSON: {0}", time, e.what());
                }
            }
            else
            {
                flog::info("Time(ms) {0}. Не удалось разобрать JSON данные.", time);
            }
        }
        else
        {
        ...
        if (Signal < 1)
        {
            // flog::info("startServer stop 1");

            stop(false);
            fs::remove(expandedPath);
        }
        else
        {
            flog::info("!!! Signal {0}, currWavFile= {1}", Signal, currWavFile);
            if (currWavFile != "")
            {
                status_direction = true;
                flog::info("curlPOST_begin currWavFile ={0}", currWavFile);
                if (!insert_REC)
                {
                    insert_REC = true;
                    if (!insert_REC_PGDB(currWavFile))
                    {
                        std::cerr << "[FAILED] Неможливо створити запис у базі\n";
                    }
                }
                // expandedPath = expandString(mainWavPath + "/" + currWavFile);
                folderSelect.setPath(wavPath);
                curr_expandedPath = expandString(folderSelect.path + "/" + currWavFile);
                // flog::info("currWavFile=", currWavFile);
                if (use_curl)
                    curlPOST_begin(currWavFile);
                else
                {
                    if (this_record)
                        curlPOST_begin_new();
...
#include <chrono>
    ...
    */
    ...
    bool insert_REC_PGDB(const std::string &oldfile)
    {
        ...
        try
        {
            // std::string quotedFile = "\"" + oldfile + "\""; // безопасно на случай пробелов
            // std::string args = "/opt/avr/scripts/db_insert.py " + quotedFile + " 1716731234 433050000 NFM 1 VHF RX1";
    ...
    // dsp::stream<dsp::complex_t>* rawIQStream = nullptr;
    ...
    // std::atomic<bool> start_short{false};
    ...
    // std::atomic<bool> stop_was_requested{false};
    // std::atomic<bool> rename_on_stop{true};
