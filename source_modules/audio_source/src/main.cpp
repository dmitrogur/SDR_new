...
SDRPP_MOD_INFO{
    /* Name:            */ "audio_source",
    /* Description:     */ "Audio source module for Aster",
    /* Author:          */ "Ryzerth",
    /* Version:         */ 0, 1, 0,
    /* Max instances    */ 1
};
...
public:
    AudioSourceModule(std::string name) {
        ...
        sigpath::sourceManager.registerSource("Аудіо", &handler);
    }

    ~AudioSourceModule() {
        stop(this);
        sigpath::sourceManager.unregisterSource("Аудіо");
    }
...
private:
    ...
    static void start(void* ctx) {
        ...
        opts.streamName = "Aster Audio Source";
    ...
    static void menuHandler(void* ctx) {
        ...
        SmGui::FillWidth();
        SmGui::ForceSync();
        if (SmGui::Button(CONCAT("Оновити##_audio_refr_", _this->name))) {
            _this->refresh();
