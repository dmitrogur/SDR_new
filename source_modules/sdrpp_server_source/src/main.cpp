...
SDRPP_MOD_INFO{
    ...
    /* Description:     */ "Malva Server source module for Aster",
    /* Author:          */ "DMH",
    /* Version:         */ 0, 2, 0,
    /* Max instances    */ 1
};
...
public:
    SDRPPServerSourceModule(std::string name) {
        ...
        sigpath::sourceManager.registerSource("Мальва-сервер", &handler);
    }

    ~SDRPPServerSourceModule() {
        stop(this);
        sigpath::sourceManager.unregisterSource("Мальва-сервер");
    }
...
private:
    ...
    static void menuHandler(void* ctx) {
        ...
        if (ImGui::InputText(CONCAT("##sdrpp_srv_srv_host_", _this->name), _this->hostname, 1023)) {
            config.acquire();
            config.conf["hostname"] = std::string(_this->hostname);
            config.release(true);
        }
        ImGui::SameLine();
        ...
        if (!connected && ImGui::Button("З'єднати##sdrpp_srv_source", ImVec2(menuWidth, 0))) {
            _this->tryConnect();
        }
        else if (connected && ImGui::Button("Відключити##sdrpp_srv_source", ImVec2(menuWidth, 0))) {
            _this->client->close();
        }
        ...
        if (connected) {
            ...
            */
            if (ImGui::Checkbox("Компресія", &_this->compression)) {
            ...
            */
            ...
            ImGui::SameLine();
            ImGui::TextColored(ImVec4(0.0f, 1.0f, 0.0f, 1.0f), "Підключено (%.3f Мбіт/с)", _this->datarate);
    ...
    void tryConnect() {
        try {
            ...
            flog::error("Could not connect to Aster: {}", e.what());
