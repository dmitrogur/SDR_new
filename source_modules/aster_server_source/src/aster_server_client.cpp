...
namespace server {
    ...
    void ClientClass::tcpHandler(int count, uint8_t* buf, void* ctx) {
        ...
        flog::info("goal {0}, _this->r_pkt_hdr->size {1}, sizeof(PacketHeader) {2}", goal, _this->r_pkt_hdr->size, sizeof(PacketHeader) );
        ...
        flog::warn("_this->r_pkt_hdr->type {0}", _this->r_pkt_hdr->type);
        if (_this->r_pkt_hdr->type == PACKET_TYPE_COMMAND) {
            ...
            flog::error("Malva Server Error: {0}", buf[sizeof(PacketHeader)]);
        }
        else {
