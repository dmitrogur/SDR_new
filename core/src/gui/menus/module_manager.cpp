...
namespace module_manager_menu {
    ...
    void draw(void* ctx) {
        ...
        if (ImGui::BeginTable("Менеджер модулів Таблиця", 3, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg, ImVec2(0, 200))) { // | ImGuiTableFlags_ScrollY 
        ...
        if (ImGui::GenericDialog("module_mgr_confirm_", confirmOpened, GENERIC_DIALOG_BUTTONS_YES_NO, []() {
                ImGui::Text("Видалення  \"%s\". Ви впевнені?", toBeRemoved.c_str());
        ...
        if (ImGui::BeginTable("Менеджер модулів Додати", 3)) {
            ImGui::TableSetupColumn("Назва");
            ImGui::TableSetupColumn("Тип");
            ...
            if (ImGui::Button("+##module_mgr_add_btn", ImVec2(btnSize.x, 0))) {
                flog::info("!!! modName={0}, modTypes[modTypeId]={1}, modTypeId={2}", modName, modTypes[modTypeId], modTypeId);
                if (!core::moduleManager.createInstance(modName, modTypes[modTypeId])) {
                    core::moduleManager.postInit(modName);
                    modified = true;
                }
                else {
                    errorMessage = "Попередження: Не вдалося створити новий екземпляр " + modTypes[modTypeId];
                    errorOpen = true;
                }
            }
