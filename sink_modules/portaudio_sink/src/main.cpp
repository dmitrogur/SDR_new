...
SDRPP_MOD_INFO{
    /* Name:            */ "audio_sink",
    /* Description:     */ "Audio sink module for Aster",
    /* Author:          */ "Ryzerth",
    /* Version:         */ 0, 1, 0,
    /* Max instances    */ 1
};
...
enum {
    ...
    AUDIO_SINK_CMD_SET_MODE,
    AUDIO_SINK_CMD_START,
    AUDIO_SINK_CMD_STOP
};
...
public:
    ...
    AudioSink(SinkManager::Stream* stream, std::string streamName) {
        ...
        core::modComManager.registerInterface("portaudio_sink", name, moduleInterfaceHandler, this);
...
private:
    ...
    static void moduleInterfaceHandler(int code, void* in, void* out, void* ctx) {
        ...
        if (code == AUDIO_SINK_GET_DEVLIST) {
            std::string* _out = (std::string*)out;
            *_out = _this->txtDevList;
        }
        /*
...
public:
    AudioSinkModule(std::string name) {
        ...
        sigpath::sinkManager.registerSinkProvider("Аудіо", provider);
    }

    ~AudioSinkModule() {
        sigpath::sinkManager.unregisterSinkProvider("Аудіо");
        Pa_Terminate();
    }
