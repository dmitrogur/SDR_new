...
namespace backend {
    ...
    int init(std::string resDir) {
        ...
        int numInstance = core::configManager.conf["InstanceNum"];// core::configManager.conf["InstanceNum"];
        std::string nameInstance = core::configManager.conf["InstanceName"];
        std::string strIcon = "/icons/sdrpp_1.png";
        strIcon = core::configManager.conf["icon"];
        int radioMode=0;
        try
        {
        ...
        std::string VERSION_Prog = "ASTER-RPM";

        switch(radioMode) {
            case 0:
                VERSION_Prog = "ASTER-RPM";
                break;
            case 1:
                VERSION_Prog = "MALVA-RPM";
                break;
            case 2:
                VERSION_Prog = "Azalea";
                break;
        ...
        window = glfwCreateWindow(winWidth, winHeight, "Aster v" VERSION_STR " (Built at " __TIME__ ", " __DATE__ ")", NULL, NULL);
        if (window == NULL)
            return 1;
        ...
        for (int i = 0; i < OPENGL_VERSION_COUNT; i++) {
            ...
            // Create window with graphics context
            monitor = glfwGetPrimaryMonitor();
            //const char* Instance make
            
            std::string strInstance = VERSION_Prog + " v" VERSION_STR " (" __TIME__ ", " __DATE__ ").     " + nameInstance;//  + " (" + std::to_string(numInstance) + ")";
            window = glfwCreateWindow(winWidth, winHeight, (const char*) strInstance.c_str(), NULL, NULL);

            // window = glfwCreateWindow(winWidth, winHeight, "Aster v" VERSION_STR " (Built at " __TIME__ ", " __DATE__ ")" + glsl_version, NULL, NULL);
            if (window == NULL) {
        ...
        */
        if (!std::filesystem::is_regular_file(resDir + strIcon)) {
            flog::error("Icon file '{0}' doesn't exist!", resDir + strIcon);
            return 1;
        }

        GLFWimage icons[10];
        // icons[0].pixels = stbi_load(((std::string)(resDir + "/icons/sdrpp.png")).c_str(), &icons[0].width, &icons[0].height, 0, 4);
        icons[0].pixels = stbi_load(((std::string)(resDir + strIcon)).c_str(), &icons[0].width, &icons[0].height, 0, 4);
    ...
    int renderLoop() {
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            if ((_winWidth != winWidth || _winHeight != winHeight) && !maximized && _winWidth > 800 && _winHeight > 800) {
                //flog::info("  winWidth = _winWidth {0}", _winWidth);
                winWidth = _winWidth;
            ...
            if ( _winWidth > 800 && _winHeight > 800) { // && winWidth > 800 && winHeight > 800
                // flog::info("winWidth{0} > 800 && winHeight > 200", winWidth);
