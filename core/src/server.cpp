...
namespace server {
    ...
    int main() {
        ...
        flog::info("startServer 1 {0}", host);
        listener = net::listen(host, port);
        flog::info("startServer 12 {0}", port);
    ...
    void _testServerHandler(uint8_t* data, int count, void* ctx) {
        ...
        flog::info("_testServerHandler, compression {0}, bb_pkt_hdr->size {1}, count {2}", compression, bb_pkt_hdr->size, count);
        if (compression) {
    ...
    void commandHandler(Command cmd, uint8_t* data, int len) {
        ...
        if (cmd == COMMAND_GET_UI) {
            ...
            flog::info("  COMMAND_SET_SAMPLE_TYPE type {0} !!!", (int) type);
    ...
    void drawMenu() {
        ...
        if (SmGui::Combo("##sdrpp_server_src_sel", &sourceId, sourceList.txt)) {
            flog::info("TRACE server! sourceId {0}:{1} Combo", sourceId, sourceList[sourceId]); 
            sigpath::sourceManager.selectSource(sourceList[sourceId]);
            core::configManager.acquire();
