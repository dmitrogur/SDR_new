...
namespace server {
    ...
    void Client::worker() {
        while (true) {
            ...
            flog::info("r_pkt_hdr->size {0}, size = {1}", r_pkt_hdr->size, r_pkt_hdr->size - sizeof(PacketHeader));
            ...
            if (r_pkt_hdr->type == PACKET_TYPE_COMMAND) {
                ...
                flog::info("decompIn writeBuf r_pkt_hdr->size {0}, szie ={1}", r_pkt_hdr->size, r_pkt_hdr->size - sizeof(PacketHeader));
