...
void IQFrontEnd::init(dsp::stream<dsp::complex_t>* in, double sampleRate, bool buffering, int decimRatio, bool dcBlocking, int fftSize, double fftRate, FFTWindow fftWindow, float* (*acquireFFTBuffer)(void* ctx), void (*releaseFFTBuffer)(void* ctx), void* fftCtx) {
    ...
    // ============= НОВЫЙ ОТЛАДОЧНЫЙ КОД =============
    if (!split.isInitialized()) { // Предполагая, что есть такой метод, или добавим его
        flog::error("FATAL: split is NOT INITIALIZED after split.init()!");
    } else {
        flog::info("DEBUG: split is INITIALIZED after split.init().");
    }
    // ===============================================
...
// В файле iq_frontend.cpp

void IQFrontEnd::bindIQStream(dsp::stream<dsp::complex_t>* stream) {
    flog::warn("IQFrontEnd::bindIQStream CALLED!");

    // Проверяем, инициализирован ли сплиттер
    if (!_init || !split.isInitialized()) {
        flog::error("FATAL: Attempted to bindIQStream, but IQFrontEnd or its splitter is not initialized!");
        // В идеале, здесь нужно бросать исключение, но для стабильности просто выйдем.
        return;
    }
    
    // ДОБАВЛЯЕМ ПРОВЕРКУ: Не пытаемся привязать уже привязанный поток
    if (split.isStreamBound(stream)) { // Вам нужно будет добавить метод isStreamBound в класс Splitter
        flog::warn("[IQFrontEnd] Stream is already bound. Ignoring bindIQStream call.");
        return;
    }
    ...
    flog::warn("IQFrontEnd::bindIQStream FINISHED SUCCESSFULLY!");
}

void IQFrontEnd::unbindIQStream(dsp::stream<dsp::complex_t>* stream) {
    if (!_init) {
        // flog::warn("IQFrontEnd::unbindIQStream called on a deinitialized object. Ignoring.");
        return;
    }
    
    // ДОБАВЛЯЕМ ПРОВЕРКУ: Не пытаемся отвязать то, чего нет
    if (!split.isStreamBound(stream)) { // Используем тот же новый метод
        // flog::warn("[IQFrontEnd] Stream is not bound. Ignoring unbindIQStream call.");
        return;
    }
...
void IQFrontEnd::bindIQStream(dsp::stream<dsp::complex_t>* stream) {
    ...
    // Проверяем, инициализирован ли сплиттер ПЕРЕД вызовом
    if (!_init) {
        flog::error("FATAL: Attempted to bindIQStream, but IQFrontEnd is not initialized!");
    }
    if (!split.isInitialized()) { // Добавьте isInitialized() в dsp::block::base, как я предлагал ранее
         flog::error("FATAL: Attempted to bindIQStream, but internal splitter is not initialized!");
    }
